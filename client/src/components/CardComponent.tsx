...
export const CardComponent: React.FC<CardComponentProps> = ({ card, className = '', animationDelay = 0 }) => {
  ...
  if (!card.faceUp) {
    return (
      <div
        style={style}
        className={`relative w-16 h-24 xs:w-20 xs:h-28 sm:w-28 sm:h-40 rounded-lg sm:rounded-xl bg-red-800 border-2 border-slate-100 shadow-xl flex items-center justify-center overflow-hidden transition-all duration-300 transform animate-deal-card ${className}`}
      >
        <div className="absolute inset-1 rounded-md border border-amber-300/40 bg-[radial-gradient(#b91c1c_2px,transparent_2px)] [background-size:6px_6px] bg-red-900 opacity-90"></div>
  ...
  return (
    <div
      style={style}
      className={`relative w-16 h-24 xs:w-20 xs:h-28 sm:w-28 sm:h-40 rounded-lg sm:rounded-xl bg-gradient-to-b from-amber-50/95 to-slate-100 border-2 border-slate-300 shadow-xl p-1.5 sm:p-2.5 flex flex-col justify-between select-none transition-all duration-300 transform animate-deal-card ${isRed ? 'text-red-600' : 'text-slate-900'} ${className}`}
    >
